...
class Gaussian4DModel:
    def __init__(self, sh_degree: int, args):
        ...
        self._xyz_gradient_accum = torch.empty(0)
    ...
    def load_ply(self, path):
        ...
        assert len(extra_f_names)==3*(self._max_sh_degree + 1) ** 2 - 3
        features_extra = np.zeros((xyz.shape[0], len(extra_f_names)))
        for idx, attr_name in enumerate(extra_f_names):
            features_extra[:, idx] = np.asarray(plydata.elements[0][attr_name])
        # Reshape (P,F*SH_coeffs) to (P, F, SH_coeffs except DC)
        features_extra = features_extra.reshape((features_extra.shape[0], 3, (self._max_sh_degree + 1) ** 2 - 1))
    ...
    def prune_points(self, mask):
        ...
        self._xyz_gradient_accum = self._xyz_gradient_accum[valid_points_mask]
        self._deformation_table = self._deformation_table[valid_points_mask]
        self._denom = self._denom[valid_points_mask]
    ...
    def densify(self, max_grad, min_opacity, extent, max_screen_size, density_threshold, displacement_scale, model_path=None, iteration=None, stage=None):
        grads = self._xyz_gradient_accum / self._denom
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter):
        self._xyz_gradient_accum[update_filter] += torch.norm(viewspace_point_tensor[update_filter,:2], dim=-1, keepdim=True)
        self._denom[update_filter] += 1
    @torch.no_grad()
